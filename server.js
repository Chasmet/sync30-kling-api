...
async function debitBalanceByType(userId, type, billedSeconds) {
  ...
  if (currentBalance < billedSeconds) {
    throw new Error("Pas assez de temps Kling disponible");
  }
...
app.get("/wallet", async (req, res) => {
  try {
    ...
    console.error("KLING WALLET ERROR:", error);
    ...
    try {
      ...
      if (balanceBeforeDebit < billedSeconds) {
        return res.status(403).json({
          ok: false,
          error: "Pas assez de temps Kling disponible",
          balanceType: BALANCE_TYPE,
      ...
      const balanceAfterDebit = await debitBalanceByType(
        userId,
        BALANCE_TYPE,
        billedSeconds
      );
      ...
      return res.json({
        ...
        billedSeconds,
        balanceType: BALANCE_TYPE,
        syncupSecondsBalance: balanceAfterDebit
      });
    } catch (error) {
      console.error("KLING SYNC START ERROR:", error);

      if (debited && billedSeconds > 0 && userId) {
        try {
          await refundBalanceByType(userId, BALANCE_TYPE, billedSeconds);
        } catch (refundError) {
          console.error("KLING START REFUND ERROR:", refundError);
        }
      }
  ...
  try {
    ...
    if (!Number.isNaN(createdAt) && Date.now() - createdAt > getJobTimeoutMs()) {
      if (job.status !== "succeeded" && job.status !== "failed") {
        ...
        return res.json({
          ...
          syncupSecondsBalance: refundedBalance
        });
      }
    }
    ...
    if (job.status === "failed") {
      return res.json({
        ...
        billedSeconds: job.billedSeconds,
        syncupSecondsBalance: job.balanceAfterRefund ?? null
      });
    }
    ...
    console.error("KLING SYNC STATUS ERROR:", error);
    return res.status(500).json({
      ok: false,
      error: safeErrorMessage(error)
    });
...
app.get("/videos", async (req, res) => {
  try {
    ...
    console.error("KLING VIDEOS ERROR:", error);
    return res.status(500).json({
      ok: false,
      error: safeErrorMessage(error)
    });
...
app.get("/open-video/:name", async (req, res) => {
  try {
    ...
    console.error("KLING OPEN VIDEO ERROR:", error);
    return res.status(500).json({
      ok: false,
      error: safeErrorMessage(error)
    });
...
app.get("/download-video/:name", async (req, res) => {
  try {
    ...
    console.error("KLING DOWNLOAD VIDEO ERROR:", error);
    return res.status(500).json({
      ok: false,
      error: safeErrorMessage(error)
    });
...
app.post("/delete-video", async (req, res) => {
  try {
    ...
    console.error("KLING DELETE ERROR:", error);
    return res.status(500).json({
      ok: false,
      error: safeErrorMessage(error)
    });
