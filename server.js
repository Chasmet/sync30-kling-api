  ...
  async (req, res) => {
    try {
      ...
      if (billedSeconds > 0) {
        ...
        if (balanceBeforeDebit < billedSeconds) {
          return res.status(403).json({
            ok: false,
            error: "Pas assez de temps standard disponible",
            balanceType: BALANCE_TYPE,
            secondsBalance: balanceBeforeDebit,
            standardSecondsBalance: balanceBeforeDebit,
      ...
      return res.json({
        ...
        standardSecondsBalance: balanceAfterDebit
      });
    } catch (error) {
  ...
  try {
    ...
    if (job.status === "succeeded") {
      return res.json({
        ...
        balanceType: job.balanceType,
        billedSeconds: job.billedSeconds || 0,
        secondsBalance: job.balanceAfterDebit,
        standardSecondsBalance: job.balanceAfterDebit
      });
    }
    ...
    if (data.status === "failed" || data.status === "canceled") {
      ...
      if ((job.billedSeconds || 0) > 0 && !job.refunded) {
        const refundedBalance = await refundBalanceByType(
          userId,
          job.balanceType,
          job.billedSeconds
        );
        job.balanceAfterDebit = refundedBalance;
        job.refunded = true;
      }
